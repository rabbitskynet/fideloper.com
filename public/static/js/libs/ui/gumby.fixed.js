...
!function() {
	...
	function Fixed($el) {
		...
		this.endPoint = Gumby.selectAttr.apply(this.$el, ['end']);
		...
		this.endPoint = this.parseAttrValue(this.endPoint);
	...
	Fixed.prototype.scroll = function() {
		...
		endPoint   = endPoint instanceof jQuery ? this.endPoint.offset().top : this.endPoint;

		// no unfix point so set to offset * 2 so offset is never higher
		// this saves having doubled up conditionals
		if(!unfixPoint) {
			unfixPoint = offset * 2;
		}
		...
		// end point specified
		if(endPoint) {
			// add end class if past end point
			if(offset >= endPoint && !this.$el.hasClass('end')) {
				this.$el.addClass('end').trigger('gumby.onEnd');
			// remove end class if before point
			} else if(offset < endPoint && this.$el.hasClass('end')) {
				this.$el.removeClass('end').trigger('gumby.onUnend');
			}
		}

		// scrolled past fixed point but less than unfixed point and no fixed class present
		if(offset >= fixedPoint && offset <= unfixPoint && !this.$el.hasClass('fixed')) {
			this.$el.addClass('fixed').trigger('gumby.onFixed');

		// scrolled past unfixed point and has fixed class present
		// or scrolled higher than specified point and has fixed class
		} else if(((offset <= fixedPoint) || (offset >= unfixPoint)) && this.$el.hasClass('fixed')) {
			this.$el.removeClass('fixed').trigger('gumby.onUnfixed');
		}
	};
	...
	Gumby.UIModule({
		module: 'fixed',
		events: ['onFixed', 'onUnfixed', 'onEnd', 'onUnend'],
		init: function() {
